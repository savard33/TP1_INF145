...
int main(void)
{
	...
	printf("nb = %d \n", nb_eq_chaine);
	equips = configuration_intiale((NB_BITS / nb_eq_chaine)*nb_eq_chaine);
	printf("nb = %s \n", bits2string(equips));
	...
	printf("\n Probabilite de bris d'equipement" );
		p_bris = get_double(MAX_PROB_BRIS, MIN_PROB_BRIS);
	printf("\nb = %lf \n", p_bris);
...
void test_configuration_initiale(void)
{
	...
	printf("\ntValeur obtenu  %s", bits2string(0xAAFFFFAA));

	rentable = chaine_est_rentable(0xAAFFFFAA,0,8);
